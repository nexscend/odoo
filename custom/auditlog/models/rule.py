...
class AuditlogRule(models.Model):
    ...
    def create_logs(
        ...
        for res_id in res_ids:
            res_name = model_model.browse(res_id).display_name
    ...
    def _prepare_log_line_vals_on_write(self, log, field, old_values, new_values):
        ...
        if log.log_type == "full" and field["relation"] and "2many" in field["ttype"]:
            ...
            if existing_ids:
                existing_values = (
                    self.env[field["relation"]].browse(existing_ids).display_name()
                )
            ...
            vals["old_value_text"] = old_value_text
            for val in vals["new_value"]:
                new_value_text = (
                    self.env[field["relation"]].browse(val).display_name
                )
                vals["new_value_text"] = new_value_text
            # new_value_text = (
            #     self.env[field["relation"]].browse(vals["new_value"]).display_name()
            # )
            # vals["new_value_text"] = new_value_text
        return vals
